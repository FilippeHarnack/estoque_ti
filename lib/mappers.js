...
export function mapEquip(r) {
  const funcionarioRaw = r.funcionario || "—";
  const funcionario = /^estoque/i.test(funcionarioRaw.trim()) ? "Estoque" : funcionarioRaw;
  const NOMES_ESTOQUE = ["estoque da t.i", "estoque ti", "sem funcionário", "sem funcionario", "ti", "estoque"];
...
export function mapMov(r) {
  ...
  else if (obs.startsWith("[transferencia] ")) {
    const dest = (r.funcionario || "").trim();
    tipo = /^estoque/i.test(dest) ? "retorno" : "transferencia";
    obs = obs.slice(16);
  }
  return {
    id:           r.id,
    data:         (r.data || hoje()).slice(0, 10),
    tipo,
    ...
    usuario:      r.operador     || "",
    funcionario:  /^estoque/i.test((r.funcionario || "").trim()) ? "Estoque" : (r.funcionario || "—"),
...
export function mapUsuario(r) {
  ...
  return {
    ...
    perfil,
    // Pode ser: URL assinada (http), data URI, path de storage (avatars/…) ou ícone padrão
    avatar:      (r.avatar?.startsWith("http") || r.avatar?.startsWith("data:image") || r.avatar?.startsWith("avatars/")) ? r.avatar : (perfil === "super_admin" ? "bolt" : perfil === "admin" ? "crown" : "user"),
    ativo:       r.ativo !== false,
